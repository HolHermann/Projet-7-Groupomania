...
//Formulaire pour le connexion à un compte
const LoginForm = () => {
  ...
  const handleLogin = (e) => {
    ...
    axios({
      ...
      .then((res) => {
        localStorage.setItem("token", res.data.token); // on insère le token dans le localstorage
        localStorage.setItem("id", res.data.userId); // On insère l'id dans le localstorage
        window.location = "/feed"; // redirection sur le fil d'actualité
      })
      .catch((err) => {
